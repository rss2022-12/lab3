...
class WallFollower:
    ...
    def __init__(self):
        self.time_plt = []
        self.error_plt = []
        self.desired_distance_plt = []
        self.actual_distance_plt = []
        # TODO:
    ...
    def get_steering_angle(self,data):
     ...
     #error signs and additional control parameter
        ...
        # Record plot vars
        self.time_plt.append(float(rospy.get_rostime().nsecs)/(10.0**9.0))
        self.error_plt.append(error)
        self.actual_distance_plt.append(np.abs(distance))
        self.desired_distance_plt.append(np.abs(self.DESIRED_DISTANCE))
...
if __name__ == "__main__":
    ...
    rospy.spin()
    plt.xlabel("Time (s)")
    plt.plot(wall_follower.error_plt, label="Error (m)")
    plt.plot(wall_follower.desired_distance_plt, label="Desired Distance (m)")
    plt.plot(wall_follower.actual_distance_plt, label="Actual Distance (m)")
    plt.title("Wall Following Error")
    plt.legend(loc="upper_left")
    plt.savefig("wall_following_error.png")
