...
class WallFollower:
    ...
    def __init__(self):
        ...
        self.error_pub = rospy.Publisher(self.ERROR_DATA, Float32, queue_size=1)
        self.desired_pub = rospy.Publisher(self.DESIRED_DATA, Float32, queue_size=1)
        self.actual_pub = rospy.Publisher(self.ACTUAL_DATA, Float32, queue_size=1)
        self.time_pub = rospy.Publisher(self.TIME_DATA, Float32, queue_size=1)
    ...
    def get_steering_angle(self,data):
     ...
     #error signs and additional control parameter
        ...
        self.desired_pub.publish(np.abs(self.DESIRED_DISTANCE))
        self.actual_pub.publish(np.abs(distance))
